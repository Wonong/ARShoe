...
namespace TFClassify
{
    public class TextureTools
    {
        ...
        public static Texture2D test(Texture2D source, int targetWidth, int targetHeight)
        {
        ...
        public static Texture2D ReflectTexture(Texture2D originTexture)
        {
            var result = reflect(
                originTexture.GetPixels32(), originTexture.width, originTexture.height);
            var resultTexture = new Texture2D(originTexture.width, originTexture.height);
            resultTexture.SetPixels32(result);
            resultTexture.Apply();
        ...
        static Color32[] reflect(Color32[] arr, int width, int height)
        {
            int i, j;
            Color32[] arr2 = new Color32[arr.Length];
            ...
            for (j = 0; j < height; j++)
            {
                for (i = 0; i < width; i++)
                {
                    arr2[(height - 1 - j) * width + i] = arr[j * width + i];
                }
